...
Automates my impractical daily gym slot reservations on [agendaqr.cl](https://www.agendaqr.cl). Logs in with RUT, picks preferred time slot, and sends a confirmation screenshot to my phone via Telegram.

## Features

- Automatically books today's gym session
...
- Configurable via a `.env` file — no credentials in code
...
The bot will log each step and send a screenshot to your Telegram when done.
...
```
0 7 * * * /home/user/miniconda3/envs/gym_bot/bin/python /home/user/gym_bot/gym_bot.py >> /home/user/gym_bot/gymbot.log 2>&1
```
...
| `03_modal.png` | After clicking today |
...
3. Finds today's date cell on the FullCalendar calendar and clicks it
